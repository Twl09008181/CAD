...
ostringstream& get_N(ostringstream&ostrm,const string&N="",vector<int>*V=nullptr);
...
void read(char *argv[],vector<gate>&Gate,vector<int>&Output_V,ostringstream&Input_N,ostringstream&Output_N,ostringstream&Wire_N)
{
    ...
    {   
        ...
        {
            get_N(Output_N,extract_num(line),&Output_V);
...
ostringstream& get_N(ostringstream&ostrm,const string&N,vector<int>*V)
{
    if(N!=""){
        if(!ostrm.str().empty())ostrm<<",";
        ostrm<<"N"<<N;
        if(V)
            (*V).push_back(stoi(N));
    }
